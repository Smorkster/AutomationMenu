...
def _compose( script_info: ScriptInfo, error_msg: str, screenshot: str, app_state: ApplicationState ) -> EmailMessage:
    ...
    to_list: list[ str ] = [ app_state.secrets.get( 'main_error_mail' ) ]

    if script_info.get_attr( 'Author' ) != None:
        get_dev_id: str = re.search( r'.*\((\w{4})\)$', script_info.get_attr( 'Author' ), re.DOTALL )
        dev: str = get_user_adobject( id = get_dev_id.group( 1 ), app_state = app_state )
    ...
    header: str = _( 'Script error' )
    text1: str = _( 'Error occured when your script ''<strong>{script_name}</strong>'' was running' ).format( script_name = script_info.get_attr( 'filename' ) )
    text2: str = _( 'The error occured at: {time}' ).format( time = datetime.now().strftime( '%Y-%m-%d %H:%M:%S' ) )
    error_title: str = _( '<strong>Error message</strong>' )
    sign: str = _( 'This is an automatic message sent from AutomationMenu' )
    mail_content: str = f"""\
    <html><head></head><body>
        <h3>{ header }</h3>
        <p>{ text1 }</strong></p>
...
def report_script_error( app_state: ApplicationState, error_msg: str , script_info: ScriptInfo = None, screenshot:str = None ) -> bool:
    ...
    try:
        msg: EmailMessage = _compose( script_info = script_info, error_msg = error_msg, screenshot = screenshot, app_state = app_state )
        server: SMTP = SMTP( app_state.secrets.get( 'smtprelay' ) )
