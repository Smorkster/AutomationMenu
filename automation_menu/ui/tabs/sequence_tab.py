...
def build_tab_content( ui: SequenceUi, add_translatable: Callable, op_callbacks: SequenceCallbacks ) -> SequenceUi:
    ...
    create_sequence_list_op_buttons( ui = ui,
                                    add_translatable = add_translatable,
                                    op_callbacks = op_callbacks )
    create_sequence_form( ui = ui,
                         add_translatable = add_translatable,
                         op_callbacks = op_callbacks )
    create_steps_display( ui = ui,
                         add_translatable = add_translatable,
                         op_callbacks = op_callbacks )
    create_sequence_editing_op_buttons( ui = ui,
                                       add_translatable = add_translatable,
                                       op_callbacks = op_callbacks )
    create_step_form( ui = ui,
                     add_translatable = add_translatable,
                     op_callbacks = op_callbacks )
...
def create_sequence_editing_op_buttons( ui: SequenceUi, add_translatable: Callable, op_callbacks: SequenceCallbacks ) -> None:
    ...
    add_step_button: Button = Button( master = sequence_ops,
                                     text = _( 'Add step' ),
                                     command = op_callbacks.add_sequence_step )
    ...
    sequence_ops.grid_columnconfigure( index = col, weight = 0 )
    save_sequence: Button = Button( master = sequence_ops,
                                   text = _( 'Save sequence' ),
                                   command = op_callbacks.save_sequence )
    ...
    sequence_ops.grid_columnconfigure( index = col, weight = 0 )
    delete_sequence: Button = Button( master = sequence_ops,
                                     text = _( 'Delete sequence' ),
                                     command = op_callbacks.delete_sequence )
    delete_sequence.grid( column = col,
                         row = 0,
                         sticky = 'nw' )
    ...
    sequence_ops.grid_columnconfigure( index = col, weight = 0 )
    abort_sequence_edit: Button = Button( master = sequence_ops,
                                         text = _( 'Abort edit' ),
                                         command = op_callbacks.abort_sequence_edit )
    abort_sequence_edit.grid( column = col,
                             row = 0,
                             sticky = 'nw' )
...
def create_sequence_form( ui: SequenceUi, add_translatable: Callable, op_callbacks: SequenceCallbacks ) -> None:
    ...
    name_title: Label = Label( master = sequence_form,
                              text = _( 'Name' ),
                              style = 'History.TLabel' )
    name_title.grid( column = 0,
                    row = row,
                    sticky = 'w' )
    ...
    description_title: Label = Label( master = sequence_form,
                                     text = _( 'Description' ),
                                     style = 'History.TLabel' )
    description_title.grid( column = 0,
                           row = row,
                           sticky = 'w' )
    ...
    description_field.grid( column = 1,
                           columnspan = 2,
                           row = row,
                           sticky = 'we' )
    ...
    stop_on_error_title: Label = Label( master = sequence_form,
                                       text = _( 'Stop on error' ),
                                       style = 'History.TLabel' )
    stop_on_error_title.grid( column = 0,
                             row = row,
                             sticky = 'w' )
    ...
    stop_on_error_field.grid( column = 1,
                             columnspan = 2,
                             row = row,
                             sticky = 'we' )
...
def create_sequence_list( ui: SequenceUi, op_callbacks: SequenceCallbacks ) -> None:
    ...
    sequence_list: Treeview = Treeview( master = ui.main_frame,
                                       columns = ( 'name', 'id' ),
                                       displaycolumns = 'name',
                                       show = '',
                                       selectmode = 'browse' )
    ...
    sequence_list.grid( column = 0,
                       row = 0,
                       sticky = 'nswe' )

    list_scrollbar: Scrollbar = Scrollbar( master = ui.main_frame )
    list_scrollbar.grid( column = 0,
                        row = 0,
                        sticky = 'nse' )
...
def create_sequence_list_op_buttons( ui: SequenceUi, add_translatable: Callable, op_callbacks: SequenceCallbacks ) -> None:
    ...
    sequence_op_frame.grid( column = 0,
                           row = 1,
                           sticky = 'we' )
    ...
    create_new_sequence: Button = Button( master = sequence_op_frame,
                                         text = _( 'Create new sequence' ),
                                         command = op_callbacks.create_new_sequence )
    create_new_sequence.grid( column = col,
                             row = 0,
                             sticky = 'nw' )
    ...
    sequence_op_frame.grid_columnconfigure( index = col, weight = 0 )
    edit_sequence: Button = Button( master = sequence_op_frame,
                                   text = _( 'Edit' ),
                                   command = op_callbacks.edit_sequence,
                                   state = 'disable' )
    edit_sequence.grid( column = col,
                       row = 0,
                       sticky = 'nw' )
    ...
    sequence_op_frame.grid_columnconfigure( index = col, weight = 0 )
    run_sequence: Button = Button( master = sequence_op_frame,
                                  text = _( 'Run selected' ),
                                  command = op_callbacks.run_sequence,
                                  state = 'disable' )
    run_sequence.grid( column = col,
                      row = 0,
                      sticky = 'nw' )
...
def create_steps_display( ui: SequenceUi, add_translatable: Callable, op_callbacks: SequenceCallbacks ) -> None:
    ...
    steps_display_frame.grid( column = 1,
                             row = 0,
                             rowspan = 3,
                             sticky = 'nswe' )
    ...
    steps_title: Label = Label( master = steps_display_frame,
                               text = _( 'Steps in sequence' ),
                               style = 'BiggerTitle.TLabel' )
    steps_title.grid( column = 0,
                     row = 0,
                     sticky = 'w' )
    ...
    display_container.grid( column = 0,
                           columnspan = 2,
                           row = 1,
                           sticky = 'nswe' )
    ...
    container_scrollbar: Scrollbar = Scrollbar( master = display_container,
                                               orient = 'vertical',
                                               command = container_canvas.yview )
    container_scrollbar.grid( column = 1,
                             row = 0,
                             sticky = 'ns' )
    ...
    window_id = container_canvas.create_window( ( 0, 0 ),
                                               window = steps_container,
                                               anchor = 'nw' )
...
def create_step_form( ui: SequenceUi, add_translatable: Callable, op_callbacks: SequenceCallbacks ) -> None:
    ...
    step_form: Frame = Frame( master = ui.main_frame,
                             style = 'SequenceStep.TFrame',
                             borderwidth = 2,
                             relief = 'solid' )
    ...
    script_title: Label = Label( master = step_form,
                                text = _( 'Script for this step' ),
                                style = 'History.TLabel' )
    script_title.grid( column = 0,
                      row = row,
                      sticky = 'nw' )
    ...
    script_list: Combobox = Combobox( master = step_form,
                                     values = script_names,
                                     state = 'readonly' )
    script_list.bind( '<<ComboboxSelected>>', op_callbacks.on_step_script_selected )
    script_list.grid( column = 1,
                     row = row,
                     padx = 5,
                     sticky = 'nw' )
    ...
    stop_on_error_title: Label = Label( master = step_form,
                                       text = _( 'Stop on error' ),
                                       style = 'History.TLabel' )
    stop_on_error_title.grid( column = 0,
                             row = row,
    ...
    input_title: Label = Label( master = step_form,
                               text = _( 'Script input parameters' ),
                               style = 'History.TLabel' )
    input_title.grid( column = 0,
                     row = row,
                     sticky = 'nw' )
    ...
    container_canvas: Canvas = Canvas( master = input_container,
                                      height = 150,
                                      highlightthickness = 0 )
    container_canvas.grid( column = 0,
                          row = 0,
                          sticky = 'we' )
    container_canvas.grid_columnconfigure( index = 0, weight = 1 )

    container_scrollbar: Scrollbar = Scrollbar( master = input_container,
                                               orient = 'vertical',
                                               command = container_canvas.yview )
    container_scrollbar.grid( column = 1,
                             row = 0,
    ...
    step_op_buttons_frame.grid( column = 1,
                               row = row,
                               sticky = 'se' )
    ...
    step_add: Button = Button( master = step_op_buttons_frame,
                              text = _( 'Save step' ),
                              command = op_callbacks.save_edited_step )
    step_add.grid( column = col,
                  row = 0,
                  sticky = 'e' )
    ...
    step_op_buttons_frame.grid_columnconfigure( index = col, weight = 0 )
    step_remove: Button = Button( master = step_op_buttons_frame,
                                 text = _( 'Remove step' ),
                                 command = op_callbacks.remove_sequence_step )
    step_remove.grid( column = col,
                     row = 0,
                     sticky = 'e' )
    ...
    step_op_buttons_frame.grid_columnconfigure( index = col, weight = 0 )
    step_abort: Button = Button( master = step_op_buttons_frame,
                                text = _( 'Abort' ),
                                command = op_callbacks.abort_add_sequence_step )
    step_abort.grid( column = col,
                    row = 0,
                    sticky = 'e' )
